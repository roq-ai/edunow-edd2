...
import { deleteEnrollmentById } from 'apiSdk/enrollments';
...
function CourseViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<CourseInterface>(
    ...
    () =>
      getCourseById(id, {
        relations: ['organization', 'user', 'enrollment'],
      }),
  );

  const enrollmentHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
        ...
        {isLoading ? (
          ...
          <>
            ...
            <Box>
              ...
              {hasAccess('enrollment', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    <Text fontSize="lg" fontWeight="bold">
                      Enrollments
                    </Text>
                    <NextLink passHref href={`/enrollments/create?course_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.enrollment?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/enrollments/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>
                              {hasAccess('enrollment', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/enrollments/edit/${record.id}`}
                                >
                              ...
                              {hasAccess('enrollment', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    enrollmentHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
